...
router.get('/applicant/:applicantId/summary', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Access control
    const accessFilter = await getAccessibleApplicantFilter(req.user!);
    const applicantRecord = await prisma.applicant.findFirst({ where: { id: applicantId, ...accessFilter } });
    if (!applicantRecord) {
      return res.status(404).json({ error: 'Applicant not found' });
    }
...
router.get('/applicant/:applicantId/mine', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Access control
    const accessFilter = await getAccessibleApplicantFilter(req.user!);
    const mineApplicant = await prisma.applicant.findFirst({ where: { id: applicantId, ...accessFilter } });
    if (!mineApplicant) {
      return res.status(404).json({ error: 'Applicant not found' });
    }
