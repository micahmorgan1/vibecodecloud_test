...
export const jobCreateSchema = z.object({
  ...
  responsibilities: z.string().max(20000).transform(sanitizeRichText).optional().nullable().or(z.literal('')),
  requirements: z.string().min(1, 'Requirements are required').max(20000).transform(sanitizeRichText),
  benefits: z.string().max(20000).transform(sanitizeRichText).optional().nullable().or(z.literal('')),
